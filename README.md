...
- [ ] _(v2)_ `GetParticipant()` - 獲取參與者詳細資訊
- [ ] _(v2)_ `GetWinner()` - 查詢中獎記錄
- [ ] _(v2)_ `GetBalance()` - 查詢合約餘額
- [ ] _(v2)_ NFT 相關查詢（`GetNFTOwner`, `GetCatInfo` 等）

**管理功能**

- [ ] _(v2)_ `SendWithdraw()` - 提取合約餘額
- [ ] _(v2)_ `SetNFTContract()` - 設定 NFT 合約地址
- [ ] _(v2)_ 完整合約管理介面
...
- [ ] _(v3)_ 結構化日志系統（logrus/zap）
- [ ] _(v3)_ Metrics 收集（Prometheus）
- [ ] _(v3)_ 優雅關閉機制
- [ ] _(v3)_ NFT 鑄造結果監控
- [ ] _(v3)_ 熔斷器與限流
- [ ] _(v3)_ 私鑰安全存儲
