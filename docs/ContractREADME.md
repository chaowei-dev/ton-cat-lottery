...
- ✅ 非擁有者執行抽獎被拒絕
- ✅ 中獎者選擇和結果記錄（新增回調機制驗證）
- ✅ 抽獎後狀態重置 (drawInProgress=false, participantCount=0, 清空參與者列表)
- ✅ 無參與者時抽獎失敗
...
**10. 錯誤處理和恢復機制測試 (8個測試)**
- ✅ `drawInProgress` 狀態追蹤正確性
- ✅ 重複抽獎嘗試防護
- ✅ 擁有者重置抽獎狀態 (`resetDrawState`)
- ✅ 非擁有者重置請求拒絕
- ✅ 超時自動重置機制 (`resetDrawStateIfTimeout`)
- ✅ NFT 鑄造回調處理正確性
- ✅ `drawStartTime` 時間戳追蹤
- ✅ 重置事件發射驗證

**11. 新合約狀態管理測試 (2個測試)**
- ✅ 新增狀態變量 (`drawInProgress`, `drawStartTime`) 存在性驗證
- ✅ 向後兼容性確保（現有功能不受影響）
...
### `Integration.test.ts` 整合測試 ✅ (10個測試案例)
...
**2. 端到端抽獎流程測試 (2個測試)**
- ✅ 完整抽獎流程：參與 → 抽獎 → 自動 NFT 鑄造（新增回調機制驗證）
  - 多用戶參與抽獎
  - 自動觸發中獎者選擇
  - 跨合約 NFT 鑄造驗證
  - 中獎者 NFT 餘額確認
...
**4. NFT 鑄造失敗和恢復測試 (4個測試)**
- ✅ NFT 合約授權失敗優雅處理
- ✅ `MintToAndNotify` 回調機制驗證
- ✅ 不足 Gas 導致 NFT 鑄造失敗處理
- ✅ 失敗操作後狀態一致性維護
...
|----------|----------|--------|------------|------|
| CatLottery 單元測試 | 56 | 100% | 100% | ✅ |
| CatNFT 單元測試 | 20 | 100% | 100% 核心功能 | ✅ |
| Integration 整合測試 | 10 | 100% | 100% 跨合約 | ✅ |
| Advanced 進階測試 | 21 | 100% | 安全性 & 效能 | ✅ |
| **總計** | **107** | **100%** | **~98%** | **✅** |

#### 🎯 測試完整性總覽
- **單元測試**: 76個案例，涵蓋所有合約方法和邊界情況（含新增錯誤處理機制）
- **整合測試**: 10個案例，驗證跨合約互動和端到端流程（含NFT失敗恢復）  
- **進階測試**: 21個案例，確保安全性和效能標準（含邊界值分析）
...
#### 1. 部署 CatNFT 合約
```bash
npx blueprint run deployCatNFT --testnet --tonconnect
```
- 部署費用：0.3 TON
- 自動檢查現有合約，可選擇使用現有或部署新合約
- 預設 4 種貓咪模板（Common, Rare, Epic, Legendary）
- 部署資訊自動保存至 `deployments/catNFT-*.json`
...
```
- 部署費用：0.2 TON
- 參與費用：0.01 TON，最大參與人數：3 人
- 自動檢查現有合約，可選擇使用現有或部署新合約
- 部署資訊自動保存至 `deployments/catLottery-*.json`

#### 3. 連接兩個合約（必要步驟）
```bash
npx blueprint run setupContracts --testnet --tonconnect
```

使用設定工具進行雙向關聯：

**3.1 選擇 "連接 CatLottery 和 CatNFT 合約"**
- 輸入 CatLottery 合約地址
- 輸入 CatNFT 合約地址

**3.2 自動執行雙向設定**
- 設定 CatNFT 的授權鑄造者 = CatLottery 合約地址
- 設定 CatLottery 的 NFT 合約 = CatNFT 合約地址
- 每個操作費用：0.05 TON
...
```
選擇 "檢查合約狀態" 來驗證：
- CatLottery 狀態：擁有者、參與費用、最大人數、抽獎狀態
- CatNFT 狀態：擁有者、授權鑄造者、總供應量、貓咪模板
...
```
- 選擇 "模擬抽獎流程" 測試參與抽獎
- 選擇 "執行抽獎" 測試中獎和 NFT 發送
- 選擇 "檢查中獎記錄" 查看結果

**部署總費用預估**：
- CatNFT 部署：0.3 TON
- CatLottery 部署：0.2 TON  
- 合約連接：0.1 TON (2 × 0.05)
- **總計：~0.6 TON**
